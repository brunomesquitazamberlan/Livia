...
def send_message_inside_context(assist_id: str, question: str):
    ...
    thread_id = retrive_last_register("pre_livia")["Item"]["thread_id"]
...
def retrive_last_register(table_name: str):
        dynamodb = boto3.resource('dynamodb', region_name='us-east-1',
                                  aws_access_key_id="AKIAQQG2YFV4HW66C4X5", aws_secret_access_key="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")  
        name = table_name
...
last_register = retrive_last_register("pre_livia")["Item"]["questions_answers_list"]
...
def main_page():
    st.set_page_config(page_title="LivIA: A inteligência Artificial da LIA", page_icon=":pencil:")

    st.title("LivIA: A inteligência Artificial da LIA")
    ...
    if st.button("Submeter"):
        with st.spinner("Processando..."):
            ...
            create_register("pre_livia", register)
            ########################################################
...
def feedback_page():
    ...
    if col1.button("👍 Sim"):
        ...
        update_is_useful_feedback("pre_livia",
                                  retrive_last_register("pre_livia")["Item"]["id_transacao"],
                                  True)
        st.rerun()
